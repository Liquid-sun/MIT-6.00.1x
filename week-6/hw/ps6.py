...
class Message(object):
    ...
    def apply_shift(self, shift):
        ...
        orig_text = self.get_message_text()
        coded_text = ""
        for ch in orig_text:
            if ch.isalpha():
                coded_text += d.get(ch)
...
class PlaintextMessage(Message):
    ...
    def get_encrypting_dict(self):
        ...
        '''
        return {k:v for k,v in self.encrypting_dict.items()}
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(-1, -27, -1):
            ...
            words = msg.split(' ')
            correct_words = len([w for w in words if is_word(self.valid_words, w)])
            res[correct_words] = msg
        best = max(res.keys())
        best_msg = res.get(best)
